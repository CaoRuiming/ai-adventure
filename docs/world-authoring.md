...
Keep `maximum_repair_attempts` at `0` or `1`; the present schema permits no higher value. `relationship_minimum` must be below `relationship_maximum`. Audit settings control local storage only; use `store_prompts = false` if retaining full prompts is not appropriate.
